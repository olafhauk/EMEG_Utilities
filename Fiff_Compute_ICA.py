...
parser.add_argument('--ChanTypes', help='Which channel types to use (eeg|meg).', nargs='+', default=['eeg', 'meg'])
...
if '.' in args.n_pca_comps:
    ...
    n_components = float(args.n_pca_comps)  # if float, select n_components by explained variance of PCA
...
to_pick = {'meg': False, 'eeg': False, 'eog': False, 'stim': False, 'exclude': 'bads'}
...
picks_meg_eeg_eog = mne.pick_types(raw.info, meg=to_pick['meg'], eeg=to_pick['eeg'],
...
### Necessary???
# # Filter raw data
# raw.filter(1., 40., n_jobs=1, fir_design='firwin', picks=picks_meg_eeg_eog) # base the ICA on 1-40 Hz filtered data, but apply it to the raw data later
...
print('###\nDefine the ICA object instance using %s. Number of PCA components based on: %s.' % (method, str(n_components)))
...
for ch_type in reject.keys():
    ...
    report.add_figs_to_section(fig_ic, captions=captions, section='ICA Components', scale=1)
...
for eog_ch in args.EOG:
    ...
    eog_epochs = create_eog_epochs(raw, ch_name=eog_ch, reject=reject)  # get single EOG trials

    eog_average = eog_epochs.average() # average EOG epochs

    inds, scores = ica.find_bads_eog(eog_epochs, ch_name=eog_ch, threshold=args.EOGthresh)  # find via correlation
    ...
    if inds != []: # if some bad components found
        ...
        fig_sc = ica.plot_scores(scores, exclude=inds, show=show)  # look at r scores of components

        report.add_figs_to_section(fig_sc, captions='%s Scores' % eog_ch, section='%s ICA component scores' % eog_ch, scale=1)
        ...
        report.add_figs_to_section(fig_rc, captions='%s Sources' % eog_ch, section='%s raw ICA sources' %eog_ch, scale=1)

        print('Plotting EOG average sources.')
        fig_so = ica.plot_sources(eog_average, exclude=inds, show=show)  # look at source time course

        report.add_figs_to_section(fig_so, captions='%s Sources' % eog_ch, section='%s ICA Sources' % eog_ch, scale=1)

        print('Plotting EOG epochs properties.')
        fig_pr = ica.plot_properties(eog_epochs, picks=inds, psd_args={'fmax': 35.},
                image_args={'sigma': 1.}, show=show)

        txt_str =  '%s Properties' % eog_ch
        captions = [txt_str for i in fig_pr]

        report.add_figs_to_section(fig_pr, captions=captions, section='%s ICA Properties' % eog_ch, scale=1)
        ...
        report.add_figs_to_section(fig_ov, captions='%s Overlay' % eog_ch, section='%s ICA Overlay' % eog_ch, scale=1)
...
if eog_inds != []: # if there are bad ECG components
    ...
    print('\n###\nUsing %d out of %d detected ICA components for EOG.' % (n_comps, len(eog_inds)))
...
for ecg_ch in args.ECG:
    ...
    ecg_epochs = create_ecg_epochs(raw, ch_name=ecg_ch, reject=reject)  # get single ECG trials

    ecg_average = ecg_epochs.average() # average ECG epochs

    # find bad ICA ECG components
    inds, scores = ica.find_bads_ecg(ecg_epochs, ch_name=ecg_ch, method=args.ECGmeth, threshold=args.ECGthresh)
    ...
    if inds != []: # if some bad components found
        ...
        report.add_figs_to_section(fig_sc, captions='%s Scores' % ecg_ch, section='%s component scores' % ecg_ch, scale=1)

        print('Plotting raw ICA sources.')
        fig_rc = ica.plot_sources(raw, exclude=inds, show=show)

        report.add_figs_to_section(fig_rc, captions='%s Sources' % ecg_ch, section='%s raw sources' % ecg_ch, scale=1)

        print('Plotting ECG average sources.')
        fig_so = ica.plot_sources(ecg_average, exclude=inds, show=show)  # look at source time course

        report.add_figs_to_section(fig_so, captions='%s Sources' % ecg_ch, section='%s ICA Sources' % ecg_ch, scale=1)

        print('Plotting ECG epochs properties.')
        fig_pr = ica.plot_properties(ecg_epochs, picks=inds, psd_args={'fmax': 35.},
                image_args={'sigma': 1.}, show=show)

        txt_str =  '%s Properties' % ecg_ch
        captions = [txt_str for i in fig_pr]

        report.add_figs_to_section(fig_pr, captions=captions, section='%s ICA Properties' % ecg_ch, scale=1)
        ...
        report.add_figs_to_section(fig_ov, captions='%s Overlay' % ecg_ch, section='%s ICA Overlay' % ecg_ch, scale=1)
...
if ecg_inds != []: # if there are bad ECG components
    ...
    print('\n###\nUsing %d out of %d detected ICA components for ECG.' % (n_comps, len(ecg_inds)))
